    ...
    for i in range(len(result_split)):
        if result_split[i][2] == 0:
           ...
           # print(current_state,current_state, next_state, next_action)
            Q[(current_state, current_action)] = Q[(current_state, current_action)] +  alpha * (gamma + gamma*  Q[(next_state,next_action)] - Q[(current_state, current_action)])
          ...
          #  print(current_state,current_state, split_state, split_action)
            Q[(current_state, current_action)] = Q[(current_state, current_action)] +  alpha * (gamma + gamma*  Q[(split_state,split_action)] - Q[(current_state, current_action)])
            policy[current_state] =  argmax(Q, current_state)
    return  Q,policy
