...
def Q_learning(deck, num, gamma=0.9):
            ...
            for double in [True, False]:
                for split in ([True, False] if player in splittable_sums else [False]):
                    ...
                    for ace_boolean in ace_boolean_list:
                        state = (player, dealer, ace_boolean, double, split)
    ...
    env = Environment(player=player_card, dealer=dealer_card, deck=deck, dealer_hidden = dealer_hidden_card)
    agent = Agent(env)
    # ←この位置にすべき
    for _ in range(num):
        real_episode = []
...
#
        ...
        s = (player_sum,dealer_sum,agent.environment.soft,agent.environment.double, agent.environment.split)#initial state
    # print(agent.environment.soft)
        while True :  # プレイヤーの行動
                # print(s)
        ...
        # print(agent.environment.soft)
            ...
            s_next = (player_sum, dealer_sum,agent.environment.soft, agent.environment.double,agent.environment.split)
       ...
       # agent.counting(dealer_hidden_card)
        ...
        while dealer_sum < 17 and player_sum < 21: # dealer turn
            ...
            if dealer_sum > 16:
                break
            if len(agent.environment.deck) < 1:
                break
        ...
        if split_bool ==True:
            s, a, r = real_episode[split_index]
            temp = list(real_episode[split_index]) 
            temp[2] = split_r  
            real_episode[split_index] = tuple(temp)
            Q, policy = Q_learning_split_update(real_episode, Q, policy, alpha=0.1, gamma=0.9)
...
if __name__ == "__main__":
    ...
    policy, Q, episode = Q_learning(deck, num=20000000, gamma=0.9)
    print(len(Q))
    ...
    save_Q_to_csv(Q, filename='July_17th_QlearningBasicStrategy_Q_table.csv')